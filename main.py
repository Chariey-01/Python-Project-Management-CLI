...
def build_parser():
    ...
    add_project_parser.add_argument("--start-date", required=True)
    add_project_parser.add_argument("--end-date", required=True)

    subparsers.add_parser("list-projects", help="Display all projects")
    ...
    complete_task_parser.add_argument("--project-id", type=int, required=True)
...
def main():
    manager = ProjectManager(load_existing=True)
    parser = build_parser()
    ...
    if args.command == "add-user":
        ...
        task = manager.add_task_to_project(
            args.project_id,
            args.title,
            args.description,
        )
